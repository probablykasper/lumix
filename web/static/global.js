...
fold("settings form", function () {

    fold("file select", function () {

        // click "select file" button opens file select dialog
        $("body").on("click", ".settings-form button.files", function () {
            $(".settings-form input#files").click();
        });
        ...
        var setupEvents = "drag dragstart dragend dragover dragenter dragleave drop";
        $(window).on(setupEvents, function (e) {
            if ($(e.target).inOrIs(".upload-avatar-container")) {
                e.preventDefault();
                e.stopPropagation();
            }
        });
        function show() {
            $(".settings-form .drop-to-select-file").removeClass("hidden");
            $(".settings-form .select-file").addClass("hidden");
        }
        function hide() {
            $(".settings-form .drop-to-select-file").addClass("hidden");
            $(".settings-form .select-file").removeClass("hidden");
        }
        // show
        $(window).on("dragenter", function (e) {
            if ($(e.target).inOrIs(".upload-avatar-container")) {
        ...
        $(window).on("dragleave", function (e) {
            e = e.originalEvent;
            if (e.relatedTarget == null || $(e.target).hasClass("upload-avatar-container")) {
                hide();
            }
        });
        // drop
        ...
        $("body").on("change", ".settings-form input#files", function (e) {
            var input = $(this);
            var files = input.prop("files");
            handleFiles(files);
        });
        ...
        function handleFiles(files) {
            ...
            reader.onload = function (e) {
                var url = e.target.result;
                $(".settings-form .account-icon-thumbnail").attr("style", "background-image: url(\"" + url + "\")");
            };
            reader.readAsDataURL(files[0]);
        }
    });
    ...
    // upload button click
    $("body").on("click", ".settings-form button.save", function () {
        save();
    });
});
